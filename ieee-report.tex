...
We find exiting places in an area before we find additional information on this location. Our implementation for finding additional information is made in such a way that extending the system with further data sources should be trivial. 
...
\section{Attribution of work}

\paragraph{J\'{a}n~Vor\u{c}\'{a}k}

\begin{itemize}
\item fetching information about objects from external sources
\item video objects visibility calculations
\item visualizing fetched data in the HTML5 video
\item evaluation
\end{itemize}

\paragraph{Frode~Tobias~Bjerke}
...
\item evaluation
\item conclusion
\item display data on map
\end{itemize}

\paragraph{Matth\"{a}us~Skiba}
\begin{itemize}
\item get POI from OpenStreetMap/Overpass API
\item draw markers on map with Openlayers API
\item set popups with additional information
\item write about the functions above
\item write abstract and introduction
\end{itemize}
